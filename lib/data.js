...
export function getSortedList() {
  ...
  const filePath = path.join(dataDir, 'nfcwest.json');
...
export async function getData(idRequested) {
  ...
  if (objMatch.length > 0) {
    ...
    // get filepath to second json file
    const filePath2 = path.join(dataDir, 'stadiums.json');

    // load second json file contents
    ...
    const objMatch2 = jsonObj2.filter(
      function(obj) {
        return obj.team.toString() === idRequested;
      }
    );
    objReturned.stadiums = objMatch2;
