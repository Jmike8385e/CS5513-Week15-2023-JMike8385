...
const dataURL = "https://dev-cc5513-week11-2023-jmike8385e.pantheonsite.io/wp-json/twentytwentyone-child/v1/orders/";
...
export async function getSortedOrderList() {
    // get filepath to json file
    // const filePath = path.join(dataDir, 'persons.json');
    ...
    return jsonObj.map(item => {
        return {
            id: item.ID.toString(),
            order_title: item.post_title,
            order_type: item.post_title
        }
    });
}
  ...
  // if you use acf without custom tables, we need transform what is a big string
    ...
    jsonObj.forEach(
      function(item) {
        ...
        let y = JSON.parse(x);
        console.log(y);
        console.log(y.reference_number);
        item.acf_fields = y;
      }
    );
